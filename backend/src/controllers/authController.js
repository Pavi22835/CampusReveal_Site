    ...
    if (!user) {
      ...
      user = await prisma.user.create({
        data: {
          ...
          role: 'STUDENT',
          credits: 50, // Welcome bonus credits
          isGuest: true,
          isVerified: true
        },
        select: {
...
const updateProfile = async (req, res) => {
  try {
    const { name, avatar, universityId, graduationYear, major } = req.body;
  ...
  try {
    const users = await prisma.user.findMany({
      ...
      select: {
        ...
        updatedAt: true,
        _count: {
          select: { reviews: true, projects: true, discussions: true }
        }
      }
    });
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      include: {
        ...
        _count: {
          select: { reviews: true, projects: true, discussions: true, comments: true }
        }
      }
    });
...
const permanentDeleteUser = async (req, res) => {
  try {
    ...
    // First delete all related records (reviews, projects, discussions, comments)
